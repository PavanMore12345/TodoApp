...
$.ajax({
    ...
    url: "http://localhost:8095/checklogin",
...
$("#register").click(function()
{
   $.ajax({
       ...
       dataType: "json",
       //headers:{"Content-Type":"application/json"},
       url: "http://localhost:8095/signup",
       success: function(data){
        if(data.status==true)
        {
          window.location.href="http://localhost:8095//index.html#login";
           location.reload();
          alert("data inserted successfully");
        }
        if(data.status==false)
        {
          window.location.href="http://localhost:8095//index.html#signup";
           location.reload();
          alert(data.message);
        }
       },
...
function validatelogin()
{
 ...
 var passval=/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[$@$!%*?&])[A-Za-z\d$@$!%*?&]{8,15}/;
 var emailval=/^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
 $('.error').hide();
 var flag=true;
if(emailid == "")
...
$("#loginPage").click(function()
{
 $.ajax({
     ...
     if(data.status == false)
     {
       window.location.href="http://localhost:8095//index.html#login";
        location.reload();
       alert("password or emailid is something wrong");
     }
      //  alert(data.status);
}
